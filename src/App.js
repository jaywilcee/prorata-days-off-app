...
import { getDaysInMonth, differenceInCalendarDays, parseISO, lastDayOfMonth, format } from 'date-fns';
...
export default function ProrataDaysOffCalculator() {
  ...
  const calculateDaysOff = (type) => {
    ...
    if (type === 'join') {
      ...
      daysWorked = differenceInCalendarDays(lastDayOfMonth(joinDate), joinDate) + 1;
    } else {
      const termDate = endDate ? parseISO(endDate) : new Date(year, monthIndex, daysInMonth);
      daysWorked = differenceInCalendarDays(termDate, new Date(year, monthIndex, 1)) + 1;
    }

    const prorataDaysOff = ((daysWorked / daysInMonth) * daysPerWeek).toFixed(2);
    setResult(prorataDaysOff);
  };

  const formattedMonth = (monthIndex) => String(monthIndex + 1).padStart(2, '0');
      ...
      <div className="max-w-md w-full bg-white shadow-xl rounded-2xl p-6 space-y-4">
        ...
        {result && (
          <div className="mt-4 p-4 bg-green-100 rounded-xl text-center">
            <strong>Prorata Days Off:</strong> {result}
          </div>
        )}
      </div>
    </div>
